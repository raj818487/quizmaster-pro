...
export class UserDashboardPage implements OnInit {
  ...
  async loadQuizzes() {
    ...
    try {
      ...
      const [assignedQuizzes, publicQuizzes, allQuizzes, userAccessRequests] =
        await Promise.all([
      ...
      this.assignedQuizzes = assignedQuizzes.map((quiz) => {
        ...
        console.log(
          `Quiz ${quiz.id} (${quiz.title}): hasAccess=${hasAccess}, isAssigned=${isAssigned}`
        );
        ...
        return {
          ...
          access_status: hasAccess
            ...
            : ('no_access' as const),
          can_start: canStart,
          display_message: canStart
            ? undefined
            : isAssigned
            ? 'Assigned but no access - contact admin for access'
            : 'Access needed - contact admin',
        };
      });
  ...
  get filteredQuizzes(): QuizWithStatus[] {
    ...
    return quizzes.sort((a, b) => {
      ...
      if (a.access_status === 'has_access' && b.access_status !== 'has_access')
        return -1;
      if (a.access_status !== 'has_access' && b.access_status === 'has_access')
        return 1;
      ...
      if (quiz.access_status === 'no_access') {
        if (quiz.assignment_status === 'assigned') {
          ...
          this.toastService.warning(
            'This quiz is assigned to you but you need access permission. Please contact an admin.'
          );
          return;
        } else {
        ...
        this.toastService.warning(
          quiz.display_message || 'You cannot start this quiz at the moment.'
        );
        return;
      }
    }
  ...
  async requestAccess(quiz: QuizWithStatus) {
    ...
    if (this.requestingAccess.has(quiz.id)) {
      this.toastService.warning(
        'Access request already in progress for this quiz.'
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        this.toastService.success(
          'Access request submitted successfully! You will be notified when an admin responds.'
        );

        // Update the quiz status immediately for better UX
        const quizIndex = this.privateQuizzes.findIndex(
          (q) => q.id === quiz.id
        );
        if (quizIndex !== -1) {
        ...
        this.toastService.error(
          'Failed to submit access request: ' + result.message
        );
      }
    } catch (error) {
  ...
  isRequestButtonDisabled(quiz: QuizWithStatus): boolean {
    ...
    return (
      this.isRequestingAccess(quiz.id) ||
      quiz.access_status === 'pending_request'
    );
  }
  ...
  viewQuizDetails(quiz: QuizWithStatus) {
    ...
    detailsMessage += `- Assignment: ${
      quiz.assignment_status === 'assigned'
        ? 'You are assigned to this quiz'
        : 'Not assigned to you'
    }\n`;
    detailsMessage += `- Access: ${
      quiz.access_status === 'has_access'
        ? 'You have access'
        : quiz.access_status === 'pending_request'
        ? 'Access request pending'
        : 'No access granted'
